...
HarmonyHubAccessory.prototype.addActivity = function(activity) {
    ...
    let service = self.accessory.addService(ActivityService(self.log), activity)
    ...
    return self.harmonyHub.client()
        ...
        .then(function(activityId) {
            _.each(self.services, function(service) {
                service.turn(service.id() == activityId)
            })
        })
        .catch(function(err) {
...
HarmonyHubAccessory.prototype.handleStateChanges = function() {
    ...
    self.log('{HarmonyHubAccessory} handle state changes for <' + self.services.length + '> services')
...
HarmonyHubAccessory.prototype.handleStateChange = function(service) {
    let self = this
    return service.handleStateChange(function() {
            self.log('{HarmonyHubAccessory} state changed for <' + service.name() + '>');
            self.updateStates()
        })
        .then(function(res) {
            let activityId = res[0]
            let callback = res[1]
            return self.harmonyHub
                .startActivityWithId(activityId)
                .then(callback)
        })
        .catch(function(err) {
            self.log.error('{HarmonyHubAccessory} handleStateChange:', err);
            throw err;
        })
}
...
HarmonyHubAccessory.prototype.setHarmonyHub = function(harmonyHub) {
    ...
    self.harmonyHub.on('connectionChanged', function() {
        self.log('{HarmonyHubAccessory} connectionChanged')
        self.loadActivities()
    })
    self.harmonyHub.on('stateChanged', function() {
        self.log('{HarmonyHubAccessory} stateChanged')
        self.updateStates()
    })
