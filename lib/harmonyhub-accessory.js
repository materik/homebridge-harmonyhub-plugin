...
HarmonyHubAccessory.prototype.addActivity = function(activity) {
    ...
    return this.accessory.addService(ActivityService(self.log), activity)
}
...
HarmonyHubAccessory.prototype.handleStateChange = function(service) {
    let self = this
    service.on('stateChanged', function(activityId, callback) {
        self.harmonyHub.startActivityWithId(activityId).then(callback)
    })
}
