...
HarmonyHub.prototype.setActivityWithId = function(on, activityId) {
	if (on) {
		return this.startActivityWithId(activityId)
	} else {
		return this.stopActivitiesIfNeeded()
	}
}

HarmonyHub.prototype.stopActivitiesIfNeeded = function() {
	let self = this;
	...
	return self.client()
		.then(function(client) {
			if (activityId > 0) {
				...
				self.log('{HarmonyHub} stop activities');
			}
