...
var PlatformAccessory = require('./platform-accessory').PlatformAccessory;
var Hub = require('./hub');
var HubConnection = require('./helper/hub-connection');
...
function ActivityAccessory(accessory, log, connection) {
	...
	PlatformAccessory.call(this, accessory, connection, ActivityAccessory.typeKey, null, log);
	
	this.log.info("{ActivityAccessory}");
}
util.inherits(ActivityAccessory, PlatformAccessory);
...
ActivityAccessory.prototype.initAsync = function() {
	...
	this.log("{ActivityAccessory} Fetching Logitech Harmony activities...");

	var self = this;
	return Hub(this.connection).getActivitiesAsync().then(function(activities, currentActivity) {
...
ActivityAccessory.prototype.updateConnection = function() {
	...
	var rtn = PlatformAccessory.prototype.updateConnection.apply(this, arguments);
...
ActivityAccessory.prototype.refreshActivityAsync = function() {
	...
	.catch(function (err) {
		self.log.error('{ActivityAccessory} Unable to get current activity with error', err);
		throw err;
	});
};
...
ActivityAccessory.prototype._setActivityServiceOn = function(service, isOn, callback, doIgnore) {
	...
	var onChange = function(args) {
		...
		self.log("{ActivityAccessory} Preemptively marking finished.");
		finish();
	};
	return this.connection
		.invokeAsync(function(client) {
			self.log("{ActivityAccessory} Switching to Activity: " + actId);
			c.addListener('change', onChange);

			var task = client.startActivity(actId);
			self.log("{ActivityAccessory} Switching Task Started: " + actId);
			return task;
		})
		.asCallback(finish)
		.finally(function(){
			self.log("{ActivityAccessory} Switch Task Finished: " + actId);
		});
};
