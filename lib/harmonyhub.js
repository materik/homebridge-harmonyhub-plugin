...
let Client = require('@harmonyhub/client-ws').getHarmonyClient;
...
let CONFIG_DEFAULT = {'explorer': {'options': {}}}
...
HarmonyHub.find = function(log, config, reset) {
    let options = _.defaultsDeep(config, CONFIG_DEFAULT).explorer.options;
    log.debug('{HarmonyHub} finding hubs with options: <' + JSON.stringify(options) + '>');

    let explorer = new Explorer(61991, options);
    return new Promise(function(resolve, reject) {
        var resolver;
        explorer.on('update', function(hubs) {
            clearTimeout(resolver)
            resolver = setTimeout(function() {
                explorer.emit('complete', hubs)
            }, 1000)
        });
        explorer.on('complete', function(hubs) {
            explorer.stop()
            resolve(hubs)
        })
        explorer.on('online', function(hub) {
            log.debug('{HarmonyHub} <' + hub.friendlyName + '> is online')
        });
        explorer.on('offline', function(hub) {
            log.warn('{HarmonyHub} <' + hub.friendlyName + '> is offline')
        });
        explorer.start();
    })
    .then(function(hubs) {
...
HarmonyHub.prototype.id = function() {
    let id = this.hub.uuid
    return HomebridgeIdentifier.isValid(id) ? id : HomebridgeIdentifier.generate(id);
};
